const API = "https://campus-wire.onrender.com";
class CampusBuzz {
    constructor() {
...
//     await this.loadCurrentUser();
//     this.setupEventListeners();   // <-- move before loadProfile
//     await this.loadProfile(Number(profileId));
...
async init() {
    ...
    if (path.includes("profile.html")) {
        ...
        // if no ?user= → open own profile automatically
        const idToLoad = profileId || this.currentUser.user_id;

        await this.loadProfile(Number(idToLoad));
    ...
    if (response.status === 401) {
        console.warn("401 received — ignoring during debug");
    ...
    try {
        ...
        if (!data || !data.user) {
            console.warn("verify-token returned empty", data);
            return;   // don't logout immediately
        }
        ...
        // TEMP: do NOT logout automatically
        // this.logout();
    }
}
    ...
    if (warnEl) warnEl.textContent = this.currentUser.warning_count;
    ...
    async loadTheme() {
        try {
            ...
            if (data.active) {
                ...
                festivalDates.textContent = theme.festival_name || "";
...
if (!container) return;   // ⭐ prevents crash on profile page
...
if (activeEl) activeEl.textContent = activeUsers;
    ...
    //         e.preventDefault();
    //         if (this.currentUser?.user_id) {
    //             this.openProfile(this.currentUser.user_id);
    //         }
    //     });
    // }
    const profileLink = document.getElementById("profileLink");
...
if (profileLink) {
    profileLink.onclick = (e) => {
        ...
        const tokenUser = this.currentUser?.user_id;

        if (tokenUser) {
            window.location.href = `/profile.html?user=${tokenUser}`;
        } else {
            // fallback if currentUser not yet loaded
    ...
    document.querySelectorAll('.emotion-filter').forEach(btn => {
        btn.addEventListener('click', (e) => {
            document.querySelectorAll('.emotion-filter')
    ...
    if (followerEl) followerEl.addEventListener("click", () => this.showFollowers());

    const followingEl = document.getElementById("followingCount");
    if (followingEl) followingEl.addEventListener("click", () => this.showFollowing());
    ...
    // --- REACTION PICKER DELEGATION ---
    document.addEventListener('click', (e) => {

        const option = e.target.closest('.reaction-option');
        if (option) {
            const picker = option.closest('.reactions-picker');
            if (!picker) return;

            const postId = picker.id.split('-')[2];
        ...
        if (!e.target.closest('.reaction-btn-container')) {
            document.querySelectorAll('.reactions-picker')
...
async loadRecentActivity(userId){
    if(!this.currentUser || this.currentUser.user_id !== userId){
        return; // prevent API call entirely
    }
    try{

        const activities = (await this.apiCall(`/activity/${userId}`)) || [];
        ...
        box.innerHTML=activities.map(a=>{
            ...
            if(a.type==="post"){ icon="📝"; title="Posted"; }
            if(a.type==="comment"){ icon="💬"; title="Commented"; }
            if(a.type==="like"){ icon="❤️"; title="Reacted"; }
            if(a.type==="follow"){ icon="👤"; title="Followed someone"; }
            ...
            return `
                <div class="activity-card">
                    ...
                    <div class="activity-header">
                        ...
                        <strong>${title}</strong>
                        <span class="activity-time">${this.getTimeAgo(new Date(a.created_at))}</span>
                    </div>

                    <div class="activity-text">
                        ${this.escapeHTML(a.text || '')}
                    </div>

                    ${
                        a.image
                        ? `<img class="activity-image" src="${a.image}" onerror="this.style.display='none'">`
                        : ""
                    }
...
openProfile(userId) {
    ...
    if (window.location.pathname.includes("profile.html")) {
        // already on profile → force reload content
...
if (btn) {
    ...
    // Load activity ONLY if owner
if (this.currentUser && this.currentUser.user_id === userId) {
    await this.loadRecentActivity(userId);
} else {
    const box = document.getElementById("recentActivity");
    if (box) box.innerHTML = "<p>This user's activity is private</p>";
}
...
async loadUserPosts(userId) {
    try {
        ...
        if (!Array.isArray(posts)) {
            console.error("Posts not array:", posts);
            return;
        }
